...
print(f"Data Loading Started...", flush = True)
...
print(f"Total files loaded: {len(audio_data)}", flush = True)
print(f"Unique sample rates: {set(sample_rates)}", flush = True)
...
for epoch in range(num_epochs):
    ...
    print(f"Epoch {epoch+1}: Train Loss= {val_loss:.4f}, Val Acc= {val_acc:.4f}, Val F1= {val_f1:.4f}, Time= {time.time()-start_time:.2f}s", flush = True)
...
plt.legend()
plt.savefig("training_history_new.png")
...
with open("classification_report_new.txt", "w") as f:
    f.write(report)
print("Evaluation completed and results saved.", flush = True)
